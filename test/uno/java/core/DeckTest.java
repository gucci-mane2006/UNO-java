 ...
 *     because that single card is the face-up top card and must stay in place.
 *     In this situation draw() throws DeckExhaustedException; GameController
 *     catches it and ends the round gracefully with no winner.
...
public class DeckTest {
    ...
    /**
     * When the draw pile is empty and the discard pile has only one card
     * (the face-up top card, which must stay in place), a reshuffle is
     * impossible. draw() must throw DeckExhaustedException so that
     * GameController can catch it and end the round gracefully.
     *
     * Note: this tests Deck in isolation. The graceful game-end behavior
     * (no winner declared, save deleted, play-again prompt shown) is
     * implemented in GameController.runRoundLoop().
     */
    @Test(expected = DeckExhaustedException.class)
    public void draw_drawPileEmpty_discardHasOneCard_throwsDeckExhausted() {
        Deck d = Deck.fromPiles(
            List.of(),
    ...
    /**
     * A completely empty deck (no draw pile, no discard pile) also throws
     * DeckExhaustedException. This is the most extreme form of exhaustion.
     */
    @Test(expected = DeckExhaustedException.class)
    public void draw_completelyEmptyDeck_throwsDeckExhausted() {
